...
class HttpAssembler {
    ...
    bool fillHead(HttpRequest& request, const char* msg) {
        ...
        if (bodyStart != std::string::npos) {
            ...
            this->headDone = true;
            // fill body
            bool ret = appendBody(request, bodyCache.c_str());
            bodyCache.clear();
    ...
    bool appendChunk(HttpRespond& respond, const char* msg) {
        ...
        bool appendChunk(HttpRequest& request, const char* msg) {
        chunkCache += msg;
        size_t length = chunkCache.size();
        while (chunkBase < length) {
            if (chunkLast == 0) {
                ...
                size_t appendLength = length - chunkBase;
                if (appendLength <= chunkLast) {
                    request.appendText(chunkCache.substr(chunkBase, appendLength));
                    chunkLast -= appendLength;
                    chunkBase = chunkPos = length;
                    return false;
                } else {
                    request.appendText(chunkCache.substr(chunkBase, chunkLast));
                    chunkBase += chunkLast + 2;
                    chunkLast = 0;
                }
                // clear cache
    ...
    bool appendBody(HttpRequest& request, const char* msg) {
        // when 'Transfer-Encoding' is 'chunked'
        if (chunkedTransferEncoding)
            return appendChunk(request, msg);
        // append to body
        request.appendText(msg);
        // when 'Content-Length' is set
        if (headerContentLength != std::string::npos) {
            if (request.size() >= headerContentLength) {
                // receive done. truncate
                request.getText().resize(headerContentLength);
                return isOK = true;
            } else {
                // continue
                return false;
...
public:
    ...
    bool append(HttpRequest& request, const char* msg) {
        if (isOK)
            return true;

        if (headDone) {
            return appendBody(request, msg);
        } else {
            return fillHead(request, msg);
        }
    }
