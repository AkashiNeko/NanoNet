...
std::pair<AddrPort, std::string> UdpServer::wait() {
    // wait for client
    char buf[4096];
    ...
    buf[len] = '\0';
    AddrPort addrPort;
    addrPort.setNetAddr(inet_addr(inet_ntoa(remote.sin_addr)));
    addrPort.setNetPort(remote.sin_port);
    // return host, port and message
    return {addrPort, std::string(buf, len)};
}

ssize_t UdpServer::send(AddrPort hostPort, std::string messages) {
    struct sockaddr_in remote;
    bzero(&remote, sizeof(remote));
    remote.sin_family = AF_INET;
    remote.sin_port = hostPort.getNetPort();
    remote.sin_addr.s_addr = hostPort.getNetAddr();
